...
root_agent = LlmAgent(
    ...
    instruction=(
        ...
        "  ],\n"
        '  "insight": "<1–3 sentences, tone-matched, kept concise>",\n'
        '  "anomalies": [\n'
        '    {"label": "<date or scope, e.g. Jan 18>", "value": "<what happened, e.g. Revenue +195%>"}\n'
        "  ],\n"
        '  "opportunities": ["<short imperative or finding>", "..."],\n'
        '  "risks": ["<short risk statement>", "..."],\n'
        '  "actions": ["<imperative phrase starting with a verb>", "..."],\n'
        '  "sql_queries": []\n'
        "}\n"
        "```\n"
        "Field rules:\n"
        "- REQUIRED: headline, metrics, insight, actions, sql_queries\n"
        "- metrics: 4–6 items for executive/client, 6+ for technical (each has label+value, "
        "delta optional)\n"
        "- actions: PLURAL array, 1–3 items. For backwards-compat, you MAY also emit a singular "
        "'action' string equal to actions[0]; the frontend will fall back to it.\n"
        "- OPTIONAL arrays (omit entirely or emit []): anomalies, opportunities, risks. The "
        "frontend hides any section whose array is empty/missing.\n"
        "- Populate optional arrays only when sub-agent data supports them. Empty arrays are "
        ...
        "TONE EFFECTS ON CONTENT (schema is identical across tones — depth and language change):\n"
        "- executive: headline ≤80 chars, 4–6 metric tiles, insight 1–2 sentences in plain "
        "business language. POPULATE THE OPTIONAL ARRAYS so the card matches ADK-baseline "
        "density: anomalies (top 3–5 dated events with magnitude), opportunities (2–3 "
        "specific replicable wins drawn from the data, e.g. 'Replicate Jan 18 success: "
        "$1,944 surge driven by ...'), risks (1–3 concrete risks with scope), actions (2–3 "
        "imperatives). No model names or SQL inside any text field.\n"
        "- technical: 6+ metric tiles including derived ratios (conversion rate, AOV, "
        "revenue/user) and secondary fields (users, page_views, sessions/user). Insight "
        "carries methodology (z-scores, model names, attribution method). Populate ALL "
        "anomalies the sub-agent surfaced (with z-scores in the value), full risk analysis, "
        "and 2–3 actions framed as analysis/extraction next steps. Compute ratios from sub-"
        "agent data rather than burying them in prose.\n"
        "- client: 4–6 warm-phrased metric tiles, no internal terms (no 'ETL', 'Markov', "
        "'z-score', 'XGBoost', 'RFM'), no SQL strings in any text field. Opportunities and "
        "actions phrased as customer-facing recommendations; omit risks unless directly "
        "client-impacting.\n\n"
        ...
        "- sub-agent 'ACTION: X' → first item in {\"actions\": [\"X\", ...]} — append "
        "complementary actions you derive from the data to reach 2–3 total\n"
        ...
        "DERIVED FIELDS (anomalies/opportunities/risks): sub-agents do not currently emit "
        "these as separate labeled sections — extract them from the sub-agent's INSIGHT "
        "prose and the underlying tool data you saw via the function-response in this "
        "conversation. Examples:\n"
        "- 'sessions z=7.98 on Jan 18 (+81.4%)' → anomalies: [{label:'Jan 18', "
        "value:'Sessions +81.4% (z=7.98)'}]\n"
        "- 'Jan 30 purchases dropped 71%' → risks: ['Jan 30 purchase volume collapsed "
        "71% — investigate checkout health']\n"
        "- 'Jan 18 spike drove $1,944 revenue' → opportunities: ['Replicate Jan 18 traffic "
        "source: drove $1,944 revenue surge']\n"
        "When chaining multiple sub-agents, merge metrics (4–6 for executive/client, 6+ "
        "for technical) and combine insights into a single 1–3 sentence summary in the "
        "orchestrator's tone. "
        "Concatenate SQL_QUERIES from all sub-agents (preserve order, dedup exact matches). "
        "Merge anomalies/opportunities/risks from all sub-agents, dedup, cap each array per "
        "the tone rules above.\n\n"
